...
# Applies DNSNameResolver CRD for OpenShift 4.17+. Required when TechPreviewNoUpgrade
# is enabled on hosted clusters, as HyperShift may not install this CRD automatically.

VERSION=$(oc get clusterversion version -o jsonpath='{.status.desired.version}')
MAJOR=$(echo "$VERSION" | cut -d. -f1)
MINOR=$(echo "$VERSION" | cut -d. -f2)

[ "$MAJOR" -eq 4 ] && [ "$MINOR" -ge 17 ] || exit 0
